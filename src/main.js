...
(function() {
  ...
  window.addEventListener('load', () => {
    ...
    if (toolContainer) {
      ...
      let toolFilters = select('#tool-flters li', true);

      on('click', '#tool-flters li', function(e) {
        e.preventDefault();
        ...
        toolIsotope.on('arrangeComplete', function() {
          AOS.refresh()
        });
      }, true);
    }
  ...
  /**
   * Initiate tool lightbox 
   */
  const toolLightbox = GLightbox({
    selector: '.tool-lightbox'
  });

  /**
   * tool details slider
   */
  new Swiper('.tool-details-slider', {
    speed: 400,
    loop: true,
    autoplay: {
      delay: 5000,
...
const myChart = new Chart(ctx, {
    ...
    data: {
        ...
        datasets: [{
            ...
            data: [200,300,200,300,200,600,700,800,900,700,500,400],
            backgroundColor: ['rgba(247, 230, 84, 1)'],
            borderWidth: 1
        }]
    },
    options: {
